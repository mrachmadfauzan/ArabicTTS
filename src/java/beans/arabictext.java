...
public class arabictext {
    ...
    public String getsendCharacter() {
        ...
        arabictext concat = new arabictext();
        try {
            concat.concatenateFiles(strlist, destination);
        } catch (Exception ex) {
            Logger.getLogger(arabictext.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private final double percent = 0.3;//biasanya digunakan nilai 0.3 (30%)
    private int dataCut;
        ...
        try {
            ...
            for (String sourceFile : sourceFilesList) {
                ...
                if (bufferTemp1 == null) {
                    ...
                    bufferTemp1 = ab.hammingWindow(bufferTemp1);
                } else {
                    if (buffer == null) {
                        bufferTemp2 = new byte[ais.available()];
                        while (true) {
                            int nBytesRead = ais.read(bufferTemp2);
                            if (DEBUG) {
                                System.out.println("read (bytes): " + nBytesRead);
                            }
                            if (nBytesRead == -1) {
                            ...
                            baos.write(bufferTemp2, 0, nBytesRead);
                        }
                        bufferTemp2 = baos.toByteArray();
                        bufferTemp2 = ab.hammingWindow(bufferTemp2);

                        dataCut = (int) percent * (Math.min(bufferTemp1.length, bufferTemp2.length));
                        byte[] avg = new byte[dataCut];

                        arabictext a = new arabictext();
                        avg = a.overlap(bufferTemp1, bufferTemp2);

                        buffer = new byte[bufferTemp1.length + bufferTemp2.length];
                        int i = 0;
                        for (int j = 0; j < bufferTemp1.length - dataCut; j++) {
                        ...
                        for (int j = dataCut; j < bufferTemp2.length; j++) {
                            buffer[i] = bufferTemp2[j];
                            i++;
                        }

                        bufferTemp1 = new byte[bufferTemp2.length];
                        System.arraycopy(bufferTemp2, 0, bufferTemp1, 0, bufferTemp2.length);
                    } else {
                        bufferTemp2 = new byte[ais.available()];
                        while (true) {
                            int nBytesRead = ais.read(bufferTemp2);
                        ...
                        buffer = new byte[buffer2.length + bufferTemp2.length];
                        int i = 0;
                        for (int j = 0; j < buffer2.length - dataCut; j++) {
                            buffer[i] = buffer2[j];
                            i++;
                        }
                        for (int j = 0; j < avg.length; j++) {
    ...
    public byte[] hammingWindow(byte[] recordedData) {

        // iterate until the last line of the data buffer
        for (int n = 1; n < recordedData.length; n++) {
            // reduce unnecessarily performed frequency part of each and every frequency
            recordedData[n] *= 0.54 - 0.46 * (Math.cos((2 * Math.PI * n)
                    / (recordedData.length - 1)));
        }
        return recordedData;
    }

    public byte[] overlap(byte[] tmp1, byte[] tmp2) {

        byte[] avg = new byte[dataCut];

        //menghitung rata-ratanya
        for (int i = 0; i < dataCut; i++) {
            avg[i] = (byte) ((tmp1[(tmp1.length - 1) - (dataCut - i)] + tmp2[i]) / 2);
        }
        return avg;
    }
